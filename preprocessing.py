...
def preprocessing_data(df):
    """
    this function perform a preprocessing on the input pandas dataframe by using some feature engineering techniques
    ...
    try:
        ...
        df['weight'] = df['item_name'].str.extract(regex, expand = False)[
            0].astype(
            int
        )
        ...
        df['sales_price'] = df['sales_price'].fillna(
            df['suggested_retail_price']
            )
        ...
        # discount
        df['discount'] = (df['sales_price'] - df['suggested_retail_price']) / \
                         df[
